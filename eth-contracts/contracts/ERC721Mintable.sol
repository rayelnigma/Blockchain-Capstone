...
contract Ownable {
    //  DONE: 1) create a private '_owner' variable of type address with a public getter function
    ...
    //  DONE: 2) create an internal constructor that sets the _owner var to the creater of the contract
    constructor() public {
        emit OwnershipTransfer(_owner, msg.sender);
        _owner = msg.sender;
    }

    //  DONE: 3) create an 'onlyOwner' modifier that throws if called by any account other than the owner.
    modifier onlyOwner() {
        require(
            msg.sender == _owner,
    ...
    //  DONE: 4) fill out the transferOwnership function
    //  DONE: 5) create an event that emits anytime ownerShip is transfered (including in the constructor)

    function transferOwnership(address newOwner) public onlyOwner {
        // DONE: make sure the new owner is a real address
        require(newOwner != address(0), "new owner cannot be 0x0 address");
